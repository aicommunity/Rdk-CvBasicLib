...
template <class T>
T* PCA_Transform(T** data, T** Eigenvector, int octave, int N, int M)
{
    T* result = new T[N];
    ...
    for(int i=0; i<N; i++)
    {
        ...
        for(int j=0; j<M; j++)
        {
            sum+= -data[i][j]*Eigenvector[octave][j];
        }
        result[i] = sum;
    }
    return result;
...
template <class T>
T PCA_TransformOne(T* data, T** Eigenvector, int octave, int M)
{
    T result = 0;
    for(int j=0; j<M; j++)
    {
        result+= -data[j]*Eigenvector[octave][j];
    }
    return result;
}
